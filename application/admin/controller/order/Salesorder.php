...
//选择进入的添加方案
    public function chooseAdd()
    {
        ...
        $result = model("CustomerResource")
            ->where("id", $receive['userid'])
            ->field("username,phone,genderdata")
            ->select();

        $result = [$result[0]['username'], $result[0]['phone'], $result[0]['genderdata']];
        

        switch ($receive['planid']) {
            case 0:
                $this->add();
                break;
            case 1:
