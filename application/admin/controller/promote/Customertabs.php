...
class Customertabs extends Backend
{
    ...
    public function index()
    {
        ...
        $this->view->assign([
           'toutiao_total'=>$this->model->with(['platform' => function ($q) {
                $q->where(['name'=>'今日头条']);
           }])->where('backoffice_id', null)->count(),

           'total_58'=>$this->model->with(['platform' => function ($q) {
                $q->where(['name'=>'58同城']);
           }])->where('backoffice_id', null)->count(),

           'baidu_total'=>$this->model->with(['platform' => function ($q) {
                $q->where(['name'=>'百度']);
            }])->where('backoffice_id', null)->count(),

           'douyin_total'=>$this->model->with(['platform' => function ($q) {
                $q->where(['name'=>'抖音']);
            }])->where('backoffice_id', null)->count()
        ]);
